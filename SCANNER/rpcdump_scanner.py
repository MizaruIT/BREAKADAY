...
class RPCDump:
    ...
    def dump(self, remoteName, remoteHost):
        ...
        print(f'Retrieving endpoint list from {remoteName}')

        entries = []

        self.__stringbinding = self.KNOWN_PROTOCOLS[self.__port]['bindstr'] % remoteName
        print(f'StringBinding {self.__stringbinding}')
        ...
        for endpoint in list(endpoints.keys()):
            # Uncomment to see the services enabled/or not
            # print("Protocol: %s " % endpoints[endpoint]['Protocol'])
            # print("Provider: %s " % endpoints[endpoint]['EXE'])
            if 'spoolsv' in endpoints[endpoint]['EXE']:
                return {"ip":remoteHost,"port":self.__port,"vulnerableToPrinterBug":True}
        ...
        return {"ip":remoteHost,"port":self.__port,"vulnerableToPrinterBug":False}
...
def scan(username, password, domain, hashes, target_ip, port):
    dumper = RPCDump(username, password, domain, hashes, int(port))
    results = dumper.dump(target_ip, target_ip)
    return results
...
if __name__ == '__main__':
    ...
    print(f"Scanning for Spooler service attack on domain:{domain}, IP: {remoteName}, and username: {username}...")
    dumper = RPCDump(username, password, domain, options.hashes, int(options.port))
    results = dumper.dump(remoteName, options.target_ip)
    if results:
