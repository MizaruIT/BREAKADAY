#!/usr/bin/python3
'''ETERNALBLUE (OR MS17-010) SCANNER'''
...
class MYSMB(smb.SMB):
    '''class with functions related to SMB protocol'''
...
def check_ms17_010(conn, target_ip, target_port):
    TRANS_PEEK_NMPIPE = 0x23
    recvPkt = conn.send_trans(pack('<H', TRANS_PEEK_NMPIPE), maxParameterCount=0xffff, maxDataCount=0x800)
    status = recvPkt.getNTStatus()
    if status == 0xC0000205:  # STATUS_INSUFF_SERVER_RESOURCES
        # print('[!] The target is not patched')
        # print(f'{target_ip}:{target_port} = VULNERABLE TO MS17-010 (target is not patched)')
        return {"ip":target_ip,"port":target_port,"vulnerableToEternalBlue":True}
    else:
        # print(f'{target_ip}:{target_port} = NOT VULNERABLE TO MS17-010 (target is patched)')
        return {"ip":target_ip,"port":target_port,"vulnerableToEternalBlue":False}
...
def scan(target_ip, port, domain='', username='', password='', hashes=None):
    try:
    ...
    results = check_ms17_010(conn, target_ip, port)
...
if __name__ == '__main__':
    ...
    print(f"Scanning for Eternal Blue attack on domain:{domain}, IP: {options.target_ip}...")
    results = check_ms17_010(conn, options.target_ip, options.port)
    if results:
