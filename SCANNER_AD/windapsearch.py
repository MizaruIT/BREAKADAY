...
class LDAPSession(object):
    ...
    def getComputerDict(self, computerResults, ipLookup=True):
        ...
        for computer in computerResults:
            ...
            for attr in computer.get_attr_names():
                computerInfo[attr] = b','.join(computer.get_attr_values(attr)).decode("utf-8") 
                # print(computerInfo[attr])
...
def prettyPrintDictionary(results, attrs=None, separator=","):
    ...
    keys = set()
    common_attrs = ['cn', 'dNSHostName', 'IP', 'userPrincipalName', 'operatingSystem', 'operatingSystemVersion',
                    'operatingSystemServicePack']
    attrs = []

    for dn, computer in results.items(): #results.iteritems():
        for key in computer:
            keys.add(key)
...
def prettywriteResults(results, filename, attrs=None, separator=","):
...
def run(args):
    ...
    print(username, password, args.dc_ip, args.domain)
    ...
    if args.computers:
        ...
        if args.output_dir:
            filename = "computers_list.csv"
            # filename = "{}/{}-computers.tsv".format(args.output_dir, startTime)
            prettywriteResults(allComputersDict, filename, attrs=searchAttrs, separator=',')
