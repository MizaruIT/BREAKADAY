#!/usr/bin/python3
'''CHECKING FOR THE VULNERABILITY: SMBLEED ON A TARGET'''
# Source: ?
...
class Smb2NegotiateRequest:
    ...
    def context(self, type_used, length):
        data_length = length
        reserved = b"\x00"*4
        return type_used + data_length + reserved
...
def connect_and_send_compressed(ip_address, port, data, offset, original_decompressed_size):
...
def connect_and_send_raw(ip_address, port, data):
    with socket.socket(socket.AF_INET) as sock:
        sock.settimeout(3)
        sock.connect((ip_address, port))
        send_negotiation(sock)
...
def test_uncompressed(ip_address, port):
    ntlm_negotiate = Smb2NtlmNegotiate().get_packet()
    session_setup = Smb2SessionSetupRequest(1, ntlm_negotiate).get_packet()
    return connect_and_send_raw(ip_address, port, session_setup)

def test_compressed_benign(ip_address, port):
    ntlm_negotiate = Smb2NtlmNegotiate().get_packet()
    session_setup = Smb2SessionSetupRequest(1, ntlm_negotiate).get_packet()
    return connect_and_send_compressed(ip_address, port, compress(session_setup), 0, len(session_setup))

def test_compressed_smbleed(ip_address, port):
    ntlm_negotiate = Smb2NtlmNegotiate().get_packet()
    session_setup = Smb2SessionSetupRequest(1, ntlm_negotiate).get_packet()
    return connect_and_send_compressed(ip_address, port, compress(session_setup), 0, len(session_setup) + 1)

def scan(ip_address, port):
    # print('[+] Sending benign uncompressed SMB packet...')
    try:
        test_uncompressed(ip_address, port)
    ...
    try:
        test_compressed_smbleed(ip_address, port)
        return {"ip":ip_address,"port":port,"vulnerableToSMBleed":True}
    except Exception as e:
        print(f'[!] {str(e).capitalize()}.\n')
    return {"ip":ip_address,"port":port,"vulnerableToSMBleed":False}
...
if __name__ == "__main__":
    ...
    port = int(sys.argv[2])
    print(f"Scanning for SMBleed attack on ip:{target_ip}, port:{port}...")
